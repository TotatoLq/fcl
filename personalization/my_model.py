...
from personalization.label_propagation_models import NonParaLP
from personalization.message_op.concat_message_op import ConcatMessageOp
...
class MyModel(nn.Module):
    ...
    def homo_init(self):
        self.homo_model = HomoPropagateModel(
            num_layers=2,
        ...
        self.total_trainable_params = round(
            sum(p.numel() for p in self.homo_model.parameters() if p.requires_grad) / 1000000,
            3,
        )
    ...
    def hete_init(self):
        ...
        self.total_trainable_params = round(
            sum(p.numel() for p in self.hete_model.parameters() if p.requires_grad) / 1000000,
            3,
        )
    ...
    def preprocess(self, adj):
        ...
        universal_re_smooth_adj = sp.coo_matrix(
            (
                torch.ones([len(self.universal_re_smooth[0])]),
...
class HetePropagateModel(nn.Module):
    def __init__(self, num_layers, feat_dim, hidden_dim, output_dim, prop_steps, dropout=0.5, bn=False, ln=False):
        ...
        self.hete_propagation = HetePropagateLayer(
            self.feat_dim, self.output_dim, self.prop_steps, self.hidden_dim, self.num_layers
        )
