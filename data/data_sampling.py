...
from data.add_edge import Ratio_Homo, Ratio_Hete
from data.utils import remove_duplicate_two_dimension_list_element


def data_partitioning(G, sampling, num_clients,ratio_train, ratio_val, ratio_test,
                      ratio_iso, ratio_homo, ratio_hete,structure_info_injection=True):
    ...
    if sampling == 'Metis':
        ...
        # graph_nx 是 networkx 图，先转换为邻接表
        adj_list = []
        for node in range(graph_nx.number_of_nodes()):
            adj_list.append(list(graph_nx.neighbors(node)))

        # 调用 pymetis
        n_cuts, membership = metis.part_graph(num_clients, adjacency=adj_list)
